...
items = source.getList('province',{},'*',[('code',1)])
for item in items:
    uri = url + item['link']
    print(uri)
    html = getHtml(uri)
    ...
    print(table)
    trs = table.find_all('tr',class_="citytr")
    for tr in trs:
        temp = {}
        tds = tr.find_all('td')
        ...
        print(temp)
        info = source.getOne('city',{'code':temp['code']},'*',[('code',1)])
        if not info:
            source.add('city',temp)
        else:
            source.modify('city',{'code':temp['code']},temp)
