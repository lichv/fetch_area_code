...
items = source.getList('town',{},'*',[('code',1)])
for item in items:
    if item['link']:
        uri = url + item['link']
        print(uri)
        ...
        if html:
            ...
            if table:
                ...
                for tr in trs:
                    ...
                    temp['type'] = tds[1].text
                    temp['name'] = tds[2].text
                    print(temp)
                    info = source.getOne('village',{'code':temp['code']},'*',[('code',1)])
                    if not info:
                        source.add('village',temp)
                    else:
                        source.modify('village',{'code':temp['code']},temp)
            else:
                print('link:'+item['link']+';villagetable')
